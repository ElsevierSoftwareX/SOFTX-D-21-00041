...
def getData():
    ...
    with requests.Session() as session:
        ...
        res = session.post(url, data={'o': '1005', 'G1045tpv104': ' Select '})
        # print(res.content)
        soup = BeautifulSoup(res.content, "html.parser")
        ...
        for group in groups:
            ...
            print('* ' + author)
            res = session.post(url, data={'m': 'tpv104', 'o': '1005', group.get('name'): ' Select '})
            soup = BeautifulSoup(res.content, "html.parser")
            ...
            for file in files:
                ...
                if file_station in stations:
                    print('\t' + file_station)
                    res = session.post(url, data={'m': 'tpv104', 'o': '1005', 'mus': mus, file.get('name'): 'Raw Data'})
                    soup = BeautifulSoup(res.content, "html.parser")
                    data = soup.find('pre')
